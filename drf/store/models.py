...
class Product(models.Model):
    ...
    slug = models.SlugField()  # Меняет ссылку товара
    description = models.TextField(null=True, blank=True, verbose_name='Описание')
    unit_price = models.DecimalField(max_digits=8, decimal_places=2, validators=[MinValueValidator(1)],
                                     verbose_name='Цена за щт')
    last_update = models.DateTimeField(auto_now_add=True, verbose_name='Время обновления')
    collection = models.ForeignKey(Collection, on_delete=models.PROTECT, related_name='products',
                                   verbose_name='Категория')
    promotion = models.ManyToManyField(Promotion, blank=True)
    inventory = models.IntegerField(default=0, verbose_name='Количество на складе')
...
class CartItem(models.Model):  # Товары Корзины
...
class OrderItem(models.Model):
    ...
    unit_price = models.DecimalField(max_digits=6, decimal_places=2, verbose_name='Цена за щт')
